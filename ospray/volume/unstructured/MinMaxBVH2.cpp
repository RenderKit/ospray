...
void MinMaxBVH::build(/*! one bounding box per primitive. The attribute value
...
void MinMaxBVH::calcAndPrintOverlap() const
{
    ...
    for(size_t i = 2; i < node.size(); i++) {
        ...
        familyCount++;
        float currentOverlap = (area(node[childIdx]) + area(node[childIdx+1])) / area(node[i]);
    ...
    std::cout << "In " << overlapsNear2 << " families there was an overlap greater than " << near2Bound << std::endl;
}
