...
namespace ospray {
  namespace amr {
    ...
    bool isActive(const AMRAccel::Leaf &leaf,
                  const float *isoValue,
                  int numIsoValues)
    {
      for (int i=0;i<numIsoValues;i++) {
        if (isoValue[i] >= leaf.valueRange.lower &&
            isoValue[i] <= leaf.valueRange.upper)
          return true;
      }
    ...
    void AMRGeometry::finalize(Model *model)
    {
      ...
      if (!isoValueData) {
        throw std::runtime_error("finalized a amrgeometry, but no"
                                 " 'isoValueData' set!");
      }
