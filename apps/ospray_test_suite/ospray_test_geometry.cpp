...
namespace {
  ...
  OSPGeometry getCylinder()
  {
    ...
    EXPECT_TRUE(data);
    ospCommit(data);
    ospSetObject(cylinder, "cylinder.position0", data);
    ospRelease(data);
    vertex.y = 1.f;
    data = ospNewData(1, OSP_VEC3F, &vertex);
    EXPECT_TRUE(data);
    ospCommit(data);
    ospSetObject(cylinder, "cylinder.position1", data);
    ospRelease(data);
  ...
  OSPGeometry getSphere()
  {
    ...
    EXPECT_TRUE(data);
    ospCommit(data);
    ospSetObject(sphere, "color", data);
    ospRelease(data);
  ...
  OSPGeometry getStreamline(bool constantRadius = true)
  {
    ...
    if (!constantRadius) {
      ...
      EXPECT_TRUE(data);
      ospCommit(data);
      ospSetObject(streamlines, "vertex.radius", data);
      ospRelease(data);
    }
    data = ospNewData(3, OSP_VEC4F, color);
    EXPECT_TRUE(data);
    ospCommit(data);
    ospSetObject(streamlines, "vertex.color", data);
    ospRelease(data);
...
TEST_P(SingleObject, simpleSphere)
{
  ...
  ospSetObject(model, "material", material);
  ospRelease(material);
  ospSetObject(model, "prim.color", data);
  ospRelease(data);
  AddModel(model);
