...
OSPGeometry createGroundPlaneGeometry(const box3f &bounds)
{
  ...
  const float z_pos     = bounds.lower.z - (0.5f * abs(center.z));
  const float xy_extend = ;

  const vec3f up   = vec3f{0.f, 1.f, 0.f};
  const vec4f gray = vec4f{0.9f, 0.9f, 0.9f, 0.75f};

  vertices.push_back(Vertex{vec3f{-planeExtent, z_pos, -planeExtent}, up, gray});
  vertices.push_back(Vertex{vec3f{planeExtent, z_pos, -planeExtent}, up, gray});
  vertices.push_back(Vertex{vec3f{planeExtent, z_pos, planeExtent}, up, gray});
  vertices.push_back(Vertex{vec3f{-planeExtent, z_pos, planeExtent}, up, gray});
...
int main(int argc, const char **argv)
{
  ...
  // ospAddGeometry(g_world, createGroundPlaneGeometry());
  ...
  auto glfwOSPRayWindow =
      std::unique_ptr<GLFWOSPRayWindow>(new GLFWOSPRayWindow(
          vec2i{1024, 768}, (box3f &)boxes.bounds, g_world, renderer));
