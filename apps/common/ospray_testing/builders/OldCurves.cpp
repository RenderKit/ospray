...
namespace ospray {
  namespace testing {
    ...
    struct OldCurves : public detail::Builder
    {
      OldCurves()           = default;
      ~OldCurves() override = default;
    ...
    void OldCurves::kleinBottle(std::vector<vec4f> &points,
                             std::vector<unsigned int> &indices,
                             std::vector<vec4f> &colors) const
    {
      // spout
    ...
    void OldCurves::embreeTutorialCurve(std::vector<vec4f> &points,
                                     std::vector<unsigned int> &indices,
                                     std::vector<vec4f> &colors) const
    {
    ...
    cpp::Group OldCurves::buildGroup() const
    {
      cpp::Geometry geom("oldCurves");
    ...
    OSP_REGISTER_TESTING_BUILDER(OldCurves, oldCurves);
