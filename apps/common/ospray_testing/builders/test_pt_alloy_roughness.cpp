...
static cpp::Material makeAlloyMaterial(const std::string &rendererType,
    vec3f color,
    vec3f edgeColor,
    float roughness)
{
  cpp::Material mat(rendererType, "alloy");
...
std::vector<cpp::Material> PtAlloyRoughness::buildMaterials() const
{
  ...
  for (int i = 0; i < dimSize; ++i) {
    materials.push_back(makeAlloyMaterial(rendererType,
        vec3f(.66f, .66f, .67f),
        vec3f(1.f, 1.f, 1.f),
        float(i) / (dimSize - 1)));
  }
