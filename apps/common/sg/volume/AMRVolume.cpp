...
struct Histogram {
  ...
  Histogram(const Range<float> &range)
    : range(range)
  {
    for (int i=0;i<N;i++)
      bin[i] = 0;
  ...
  void print() {
    for (int i=0;i<N;i++) {
      ...
      float hi = range.lower + (range.upper-range.lower)*float(i+1)/N;
      printf("bin[%3i] { %8.4f-%8.4f } = %9li (%3i%%)\n",
             i,lo,hi,bin[i],int(bin[i]*100.f/total));
    }
  };
  Range<float> range;
  size_t bin[N];
...
void importAMR(const FileName &fileName,
...
void AMRVolume::parseRaw2AmrFile(const FileName &fileName, int BS, int maxLevel)
{
  char *maxLevelEnv = getenv("AMR_MAX_LEVEL");
  if (maxLevelEnv) {
    maxLevel = atoi(maxLevelEnv);
    ospLogF(1) << "will only parse amr file up to level " << maxLevel << std::endl;
  }
  ...
  ospLogF(1) << "read file; found " << brickInfo.size() << " bricks" << endl;
...
void AMRVolume::preCommit(RenderContext& ctx)
{
  ...
  for (int bID=0;bID<brickInfo.size();bID++) {
    ...
    OSPData data = ospNewData(bi.size().product(),OSP_FLOAT,
                              this->brickPtrs[bID],OSP_DATA_SHARED_BUFFER);
  ...
  ospSet2f(volume, "voxelRange", valueRange.toVec2f().x,valueRange.toVec2f().y);
  ...
  ospCommit(volume);
  ospAddVolume((OSPModel)ctx.world->valueAs<OSPObject>(),(OSPVolume)volume);
}
                              ...
                              const unsigned char *binBasePtr)
{
  ...
  if (!clampRangeString.empty()) {
    sscanf(clampRangeString.c_str(),"%f %f",&clampRange.lower,&clampRange.upper);
  }
  if (fileName != "") {
    ...
    if (realFN.ext() == "hdf5") {
      ...
      parseAMRChomboFile(nodePtr, realFN,compName,
      ...
      if (BSs == "")
        throw std::runtime_error("no 'brickSize' specified for raw2amr generated file");
      int BS = atoi(BSs.c_str());
      parseRaw2AmrFile(realFN,BS);
    }
  }
  else throw std::runtime_error("no filename set in xml node...");
}
