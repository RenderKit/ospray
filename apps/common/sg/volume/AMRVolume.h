...
namespace ospray {
  ...
  //! AMR SG node with Chombo style structure
    struct AMRVolume : public Volume {
      ...
      void parseRaw2AmrFile(const FileName &fileName, int BS, int maxLevel=1<<30);
    ...
    extern void parseAMRChomboFile(std::shared_ptr<sg::AMRVolume> &node, const FileName &fileName,
                  const std::string &desiredComponent,
                  const Range<float> *clampRange, int maxLevel=1<<30);
#endif
