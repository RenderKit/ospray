    ...
    struct Metal : public ospray::Material {
      ...
      virtual void commit() {
        ...
        const vec3f& reflectance
          = getParam3f("reflectance",vec3f(0.19,0.45,1.5));
        const vec3f& eta
          = getParam3f("eta",vec3f(.4f,0.f,0.f));
        const vec3f& k
          = getParam3f("k",vec3f(3.06,2.4,1.88));
        const float roughness
          = getParamf("roughhness",0.005);
        
        ispcEquivalent = ispc::PathTracer_Metal_create
          ((const ispc::vec3f&)reflectance,(const ispc::vec3f&)eta,
           (const ispc::vec3f&)k,roughness);
      }
    };
