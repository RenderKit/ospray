...
namespace ospray {
  ...
  LiveTileOperation::LiveTileOperation(DFB *dfb,
                                   const vec2i &begin,
                                   size_t tileID,
                                   size_t ownerID)
  ...
  {
    // accumulate, compute the finished normalized colors, and compute the error,
    // but don't write the finished colors into the color buffer yet because pixel
