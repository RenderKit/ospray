...
OPTION(OSPRAY_SUPPORT_DOUBLES OFF "Build double precision support?")


MACRO(OSPRAY_CONFIGURE_STREAM)
  SET(OSPRAY_STREAM_ISA "NATIVE" CACHE STRING
      "Target ISA (NATIVE SCALAR, SSE4, AVX, AVX2, KNL, SKX)")
ENDMACRO()
...
MACRO(OSPRAY_CREATE_SIMD_LIBRARY LIBRARY_BASE_NAME)
  ...
  IF (OSPRAY_STREAM_ISA STREQUAL "ALL")
    SET(STREAM_ISALIST  sse4 avx avx2 skx knx)
  ELSEIF (OSPRAY_STREAM_ISA STREQUAL "SCALAR")
    SET(STREAM_ISALIST scalar)
  ELSEIF (OSPRAY_STREAM_ISA STREQUAL "NATIVE")
    SET(STREAM_ISALIST native)
  ELSEIF (OSPRAY_STREAM_ISA STREQUAL "SSE4")
    SET(STREAM_ISALIST scalar sse4)
  ELSEIF (OSPRAY_STREAM_ISA STREQUAL "AVX")
    SET(STREAM_ISALIST scalar sse4 avx )
  ELSEIF (OSPRAY_STREAM_ISA STREQUAL "AVX2")
    SET(STREAM_ISALIST scalar sse4 avx avx2)
    #    SET(ISALIST sse avx avx2 skx knx)
  ELSE ()
    MESSAGE(FATAL_ERROR "un-recognized ospray_build_isa '${OSPRAY_STREAM_ISA}' in ospray_create_simd_library")
  ENDIF()

  SET(OSPRAY_CXXFLAGS_COMMON "")
  IF (OSPRAY_COMPILER_ICC)
    SET(OSPRAY_CXXFLAGS_SCALAR -DOSPRAY_SCALAR=1)
    SET(OSPRAY_CXXFLAGS_NATIVE -march=native -DOSPRAY_SIMD_NATIVE=1)
    SET(OSPRAY_CXXFLAGS_SSE4 -xsse4.2 -DOSPRAY_SIMD_SSE=1)
    SET(OSPRAY_CXXFLAGS_AVX  -xAVX -DOSPRAY_SIMD_AVX=1)
    SET(OSPRAY_CXXFLAGS_AVX2 -xCORE-AVX2 -DOSPRAY_SIMD_AVX2=1)
    SET(OSPRAY_CXXFLAGS_SKX  -xCORE-AVX512 -DOSPRAY_SIMD_SKX=1)
    SET(OSPRAY_CXXFLAGS_KNL  -xMIC-AVX512 -DOSPRAY_SIMD_KNX=1)
  ELSEIF (OSPRAY_COMPILER_GCC)
#TODO: fix tsimd to work without any isa in gcc mode - for now, i'm adding sse4 even for scalar
    SET(OSPRAY_CXXFLAGS_NATIVE -march=native -DOSPRAY_SIMD_NATIVE=1)
    SET(OSPRAY_CXXFLAGS_SCALAR -msse4.2 -DOSPRAY_SIMD_NONE=1 -DOSPRAY_SCALAR=1)
    SET(OSPRAY_CXXFLAGS_SSE4 -msse4.2 -DOSPRAY_SIMD_SSE=1)
    SET(OSPRAY_CXXFLAGS_AVX  -mavx -DOSPRAY_SIMD_AVX=1)
    SET(OSPRAY_CXXFLAGS_AVX2 -mavx2 -mf16c -mfma -mlzcnt -mbmi -mbmi2 -DOSPRAY_SIMD_AVX2=1)
    SET(OSPRAY_CXXFLAGS_SKX   -mavx512f -mavx512dq -mavx512cd -mavx512bw -mavx512vl -mf16c -mavx2 -mfma -mlzcnt -mbmi -mbmi2 -DOSPRAY_SIMD_SKX=2)
    SET(OSPRAY_CXXFLAGS_KNL  -mavx512f -mavx512pf -mavx512er -mavx512cd -mf16c -mavx2 -mfma -mlzcnt -mbmi -mbmi2 -DOSPRAY_SIMD_KNX=1)
  ELSEIF (OSPRAY_COMPILER_CLANG)
    ...
    SET(OSPRAY_CXXFLAGS_AVX  -mavx -DOSPRAY_SIMD_AVX=1)
    SET(OSPRAY_CXXFLAGS_AVX2 -mavx2 -mf16c -mfma -mlzcnt -mbmi -mbmi2 -DOSPRAY_SIMD_AVX2=1)
    SET(OSPRAY_CXXFLAGS_SKX -march=skx -DOSPRAY_SIMD_SKX=1)
    SET(OSPRAY_CXXFLAGS_KNL -march=knl -DOSPRAY_SIMD_KNX=1)
  ELSE()
    MESSAGE("OSPRAY_CREATE_SIMD_LIBRARY: Compiler not recognized!")
  ENDIF()

  # the name of the library we'll be generating for that isa
  SET(OSPRAY_SIMD_LIBNAME_SCALAR scalar)
  SET(OSPRAY_SIMD_LIBNAME_NATIVE native)
  SET(OSPRAY_SIMD_LIBNAME_SSE4 sse4)
  SET(OSPRAY_SIMD_LIBNAME_AVX  avx)
  SET(OSPRAY_SIMD_LIBNAME_AVX2 avx2)
  SET(OSPRAY_SIMD_LIBNAME_KNL knl)
  SET(OSPRAY_SIMD_LIBNAME_SKX skx)
  ...
  FOREACH(_ISA ${STREAM_ISALIST})
    STRING(TOUPPER ${_ISA} ISA)
    message("building for ISA ${ISA}")
    ...
    OSPRAY_ADD_LIBRARY(${LIBRARY_NAME}_float SHARED ${LIBRARY_SOURCES})
    message("building ${LIBRARY_NAME}_float from ${LIBRARY_SOURCES} flags  ${OSPRAY_CXXFLAGS_${ISA}}")
    TARGET_COMPILE_OPTIONS(${LIBRARY_NAME}_float PRIVATE ${OSPRAY_CXXFLAGS_COMMON} ${OSPRAY_CXXFLAGS_${ISA}} -DOSPRAY_USE_DOUBLES=0)
    TARGET_LINK_LIBRARIES(${LIBRARY_NAME}_float ${LIBRARY_LIBS})
    OSPRAY_SET_LIBRARY_VERSION(${LIBRARY_NAME}_float)
#    IF(${LIBRARY_EXCLUDE_FROM_ALL})
#      SET_TARGET_PROPERTIES(${LIBRARY_NAME}_float PROPERTIES EXCLUDE_FROM_ALL TRUE)
#    ELSE()
#      OSPRAY_INSTALL_LIBRARY(${LIBRARY_NAME}_float ${LIBRARY_COMPONENT})
#    ENDIF()
