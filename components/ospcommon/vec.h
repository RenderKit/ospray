...
namespace ospcommon {
  ...
  struct vec_t<T, 2> : public vec_base
  {
    ...
    template <typename OT>
    inline vec_t(const vec_t<OT, 2> &o) : x(o.x), y(o.y)
    {
    }
  ...
  struct vec_t<T, 3> : public vec_base
  {
    ...
    template <typename OT>
    inline vec_t(const vec_t<OT, 3> &o) : x(o.x), y(o.y), z(o.z)
    {
    }
  ...
  struct vec_t<T, 4> : public vec_base
  {
    ...
    template <typename OT>
    inline vec_t(const vec_t<OT, 4> &o) : x(o.x), y(o.y), z(o.z), w(o.w)
    {
    }
...
#define binary_operator(name, op)                                              \
  ...
  {                                                                            \
    return vec_t<T, 2>(a.x op b.x, a.y op b.y);                                \
                                                                               ...
                                                                               \
  template <typename T>                                                        \
  inline vec_t<T, 3> name(const vec_t<T, 3> &a, const vec_t<T, 3> &b)          \
  {                                                                            \
  ...
  inline auto name(const vec_t<T1, N> &v1, const vec_t<T2, N> &v2)             \
      ->vec_t<decltype(T1() op T2()), N>                                       \
  {                                                                            \
    using result_t = vec_t<decltype(T1() op T2()), N>;                         \
  ...
  template <typename T,                                                        \
      typename U = T,                                                          \
      typename = traits::is_valid_scalar_for_binary_op_t<vec_t<T, 3>, U>>      \
  inline auto name(const vec_t<T, 3> &a, const U &b)                           \
      ->vec_t<decltype(T() op U()), 3>                                         \
  {                                                                            \
    using result_t = vec_t<decltype(T() op U()), 3>;                           \
  ...
  template <typename T,                                                        \
      typename U = T,                                                          \
      typename = traits::is_valid_scalar_for_binary_op_t<vec_t<T, 3>, U>>      \
  inline auto name(const U &a, const vec_t<T, 3> &b)                           \
      ->vec_t<decltype(U() op T()), 3>                                         \
  {                                                                            \
    using result_t = vec_t<decltype(U() op T()), 3>;                           \
...
#define binary_operator(name, op)                                              \
  ...
  {                                                                            \
    ...
    return a;                                                                  \
                                                                               ...
                                                                               \
  template <typename T, typename U>                                            \
  inline vec_t<T, 3> &name(vec_t<T, 3> &a, const vec_t<U, 3> &b)               \
  {                                                                            \
    a.x op b.x;                                                                \
    a.y op b.y;                                                                \
    a.z op b.z;                                                                \
  ...
  {                                                                            \
    ...
    return a;                                                                  \
                                                                               ...
                                                                               \
  template <typename T, typename U, typename = traits::is_arithmetic_t<U>>     \
  inline vec_t<T, 3> &name(vec_t<T, 3> &a, const U &b)                         \
  {                                                                            \
    a.x op b;                                                                  \
    a.y op b;                                                                  \
    a.z op b;                                                                  \
...
#undef binary_operator
  ...
  // -------------------------------------------------------
  template <typename T>
  inline vec_t<T, 3> madd(
      const vec_t<T, 3> &a, const vec_t<T, 3> &b, const vec_t<T, 3> &c)
  {
    return vec_t<T, 3>(
  ...
  template <typename T>
  inline bool anyLessThan(const vec_t<T, 3> &a, const vec_t<T, 3> &b)
  {
    return a.x < b.x || a.y < b.y || a.z < b.z;
  }
  ...
  // -------------------------------------------------------
  template <typename T, int N>
  inline T length(const vec_t<T, N> &v)
  {
    return sqrt(dot(v, v));
  }
  ...
  // -------------------------------------------------------
  template <typename T, int N>
  inline vec_t<T, N> normalize(const vec_t<T, N> &v)
  {
    return v * rsqrt(dot(v, v));
  }

  template <typename T, int N>
  inline vec_t<T, N> safe_normalize(const vec_t<T, N> &v)
  {
  ...
  template <typename T, int N>
  inline vec_t<T, N> interpolate_uv(const vec_t<float, 3> &f,
      const vec_t<T, N> &a,
      const vec_t<T, N> &b,
      const vec_t<T, N> &c)
  {
...
#undef define_functor
  ...
  // -------------------------------------------------------
  template <typename T>
  inline T reduce_add(const vec_t<T, 2> &v)
  {
    return v.x + v.y;
  }
  template <typename T>
  inline T reduce_add(const vec_t<T, 3> &v)
  {
    return v.x + v.y + v.z;
  }
  template <typename T>
  inline T reduce_add(const vec_t<T, 4> &v)
  {
    return v.x + v.y + v.z + v.w;
  }

  template <typename T>
  inline T reduce_mul(const vec_t<T, 2> &v)
  {
    return v.x * v.y;
  }
  template <typename T>
  inline T reduce_mul(const vec_t<T, 3> &v)
  {
    return v.x * v.y * v.z;
  }
  template <typename T>
  inline T reduce_mul(const vec_t<T, 4> &v)
  {
    return v.x * v.y * v.z * v.w;
  }

  template <typename T>
  inline T reduce_min(const vec_t<T, 2> &v)
  {
    return min(v.x, v.y);
  }
  template <typename T>
  inline T reduce_min(const vec_t<T, 3> &v)
  {
    return min(min(v.x, v.y), v.z);
  }
  template <typename T>
  inline T reduce_min(const vec_t<T, 4> &v)
  {
    return min(min(v.x, v.y), min(v.z, v.w));
  }

  template <typename T>
  inline T reduce_max(const vec_t<T, 2> &v)
  {
    return max(v.x, v.y);
  }
  template <typename T>
  inline T reduce_max(const vec_t<T, 3> &v)
  {
    return max(max(v.x, v.y), v.z);
  }
  template <typename T>
  inline T reduce_max(const vec_t<T, 4> &v)
  {
...
namespace std {
  ...
  template <typename T>
  struct less<ospcommon::vec_t<T, 3>>
  {
    inline bool operator()(
        const ospcommon::vec_t<T, 3> &a, const ospcommon::vec_t<T, 3> &b) const
    {
      return (a.x < b.x) ||
