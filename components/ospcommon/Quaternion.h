...
namespace ospcommon
{
  ...
  struct QuaternionT
  {
    ...
    __forceinline explicit QuaternionT( const Vector& v ) : r(zero), i(v.x), j(v.y), k(v.z) {}
    __forceinline          QuaternionT( const T& r, const T& i, const T& j, const T& k ) : r(r), i(i), j(j), k(k) {}
    __forceinline          QuaternionT( const T& r, const Vector& v ) : r(r), i(v.x), j(v.y), k(v.z) {}
    ...
    static __forceinline QuaternionT rotate(const Vector& u, const T& r) {
      return QuaternionT<T>(cos(T(0.5)*r),sin(T(0.5)*r)*normalize(u));
    }

    /*! returns the rotation axis of the quaternion as a vector */
    __forceinline const Vector v( ) const { return Vector(i, j, k); }
  ...
  template<typename T> __forceinline typename QuaternionT<T>::Vector       operator *( const QuaternionT<T>& a, const typename QuaternionT<T>::Vector      & b ) { return (a*QuaternionT<T>(b)*conj(a)).v(); }
  ...
  template<typename T> __forceinline typename QuaternionT<T>::Vector 
